...
I will be replicating analysis of the 3D landmark and linear measurement data, but not the replication analyses.
...
The returned object (Pro_sup) is another list that contains a bunch of different objects. Some of these include an array of coordinates (coords), a vector of centroid sizes for each specimen (Csize), a variance-covariance matrix of Procrustes shape variables (points.VCV), and a Procrustes distance matrix (procD).

Next, I need to project these coordinates onto a "linear tangent space." Looks like this is plotting coordinates on principal component axes. From some googling, it looks like there was a function directly for this in early versions of the geomorph package, but it is now depreciated (from v. 3.3.0 on) and they suggest gm.prcomp instead. This function defaults to using an OLS for centering, which I think the authors used (instead of GLS), but I am not 100% sure because they aren't specific in the manuscript. I also print out a summary and plot the amount of variance explained by each PC.
...
ggplot() + 
  geom_polygon(data=hull.data,aes(x=Comp1,y=Comp2,group=Pop),alpha=0.30) + # add the convex hulls
...
When you look at the anova table you can see that the interaction of centroid size and population ID is not significant, which matches what they find.


To replicate figure S2, I have to use different criterion to display shape scores (PredLine - based on fitted values and RegScore - based on regression coefficients). These are plotted below and compared with the figure from the supplementals. 
```{r}
...
Okay, so now, I can run the permutation-based Procrustes MANOVA to regress tangent space coordinate on population identity and sex. Because of the above results they only control for isometric size (aka do nothing). This will also be the case for the linear measurement MANOVA. 
...
My anova table looks very similar to the one that they report except that the standard deviation of the F statistic is different. I am not sure why. I have had to basically play around with the inputs for the linear model call to make the values match up as much as possible. I am not 100% sure if this is because I don't understand something explained in the paper (such as what inputs should be included in this line of code if you are using GLS rather than OLS) or if they were just not specific enough for me to know right off the bat what I was supposed to do. At a minimum, I get the same regression coefficients so I guess that is good.
...
They also did pairwise comparisons within groups by looking at the dispersion of residuals around the mean shape for each population and using that to group shape variance. I am not going to do that for now. Honestly, I am not really able to do the other modelling that they do because they use a GLS estimator which requires me to include some sort of covariance matrix and I don't know where I generate one of those in the analysis pipeline and from what it is generated. Very frustrating.

So I am going to move on to looking at the linear measurements for all the specimen. Below, I load in the data as a tibble and show a few lines of raw code.
...
Here I wil normalize the 6 linear measurements and cube root of endocranial volume by centroid size (a proxy for overall cranial size). Below, I create a dataframe with the normalized measurements and I look at the correlation between variables.
```{r}
...
Finally, we can plot the differences among populations in centroid size, and 3 normalized linear skull measurements.
...
These plots look like the ones in the paper except that I have not yet run the models to give me the significance.